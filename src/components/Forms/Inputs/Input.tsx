...
export function Input({ name, type, icon, variant = "", isRequired, inputLink, value = "", label = "", mask = "", register = undefined, onChange, inputRef, control, error, maxW, colorScheme, ...rest }: FormInputProps){
    ...
    return icon ? (
        <FormControl pos="relative" isInvalid={!!error} maxW={maxW}>
            {
                label && (
                    <FormLabel zIndex="1" cursor="text" color="white" transition="ease 0.2s" pos="absolute" fontWeight="normal" fontSize={"18px"} top={controlledValue === "" ? "14px" : "6px"} _focus={{top: "6px", fontSize: "10", color: "white"}} left="55px">{label}{isRequired && '*'}</FormLabel>
                )
            }

            <InputGroup pos="relative">
                <InputLeftElement w="55px" h="50px" pointerEvents="none" ><Icon as={icon}  stroke={error ? "#e53e3e" : (controlledValue === "" ? "#ffffff" : "#7FCEFF")} fill="none" w="18" h="18px" strokeWidth="3"/></InputLeftElement>

                <ChakraInput {...getControlledInputAttributes()} fontWeight="semibold" pt="14px" name={name} h="50px" pl="55px" type={type} fontSize="sm" borderColor={variant === 'outline' ? ( colorScheme === 'dark' ? "gray.700"  : "gray.400" ) : "transparent"} bgColor={"#ffffff"} _hover={{borderColor: 'none', bg: 'gray.200'}} _focus={{borderColor:"blue.500", boxShadow: "0 3px 10px 0 rgb(34 41 47 / 10%)"}} size="lg" _placeholder={{color: "gray.600"}} border="1px" borderRadius="3px" {...rest}/>
            </InputGroup>
    ...
    (
        <FormControl pos="relative" isInvalid={!!error} maxW={maxW}>
            ...
            <ChakraInput
                ...
                name={name} h="50px" pl="4" py="3" type={type} fontWeight="semibold" fontSize="md" borderTop={"none"} borderX={"none"} borderBottom={"2px solid #d9d9d9"} bgColor={"#ffffff"} size="lg" _hover={{borderColor: 'none', bg: 'gray.200'}} _focus={{borderColor:"black", boxShadow: "0 3px 10px 0 rgb(34 41 47 / 10%)"}} _placeholder={{color: "gray.600", fontWeight: "regular"}} borderRadius="0px" {...rest}
            />
