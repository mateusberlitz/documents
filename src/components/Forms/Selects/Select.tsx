...
export function Select({ name, children, variant, selectRef, value = "", label, isRequired, selected, error, register, onChange, ...rest } : FormSelectProps){
    ...
    return(
        <FormControl pos="relative" isInvalid={!!error}>
            ...
            <ChakraSelect {...getRegister()} borderRadius="3px" h="50px" fontWeight={"semibold"} name={name} fontSize="md" bgColor={"#ffffff"} borderTop={"none"} borderBottom={"2px solid #d9d9d9"} _focus={{borderColor: "redmain"}} borderX="none" _hover={ {bgColor: '#fffffff'} } size="lg" color={controlledValue ? "blue.primary" : "gray.700"} {...rest}>
                {children}
            </ChakraSelect>
